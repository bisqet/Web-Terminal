export class FileSystem extends EventTarget {
    constructor({name = '', type = 'emulated', rootDir, path, updateFSName = false}) {
        ...
        if (this.rootDir instanceof Promise) rootDir.then((res) => {// allow creation of filesystem not waiting for rootDir to be ready
            this.rootDir = res;
...
const createRootDir =  (type) => new Promise((res,rej)=>{
    if (type === 'emulated') return navigator.storage.getDirectory().then(res);
    if (type === 'local') return window.showDirectoryPicker().then(res);
...
export class SuperRootDir extends Map {
    ...
    set(name, fileSystem) {
        if(fileSystem.updateFSName===true) fileSystem.addEventListener('nameUpdated', (event)=>{
